...
class CoordAtt(nn.Module):
    """Coordinate Attention Module - embeds location information into channel attention.

    Uses 1D directional pooling (horizontal and vertical) instead of 2D global pooling,
    which is particularly effective for capturing edges and features of small objects.
    ...
    def forward(self, x):
        ...
        # Get horizontal and vertical directional features
        x_h = self.pool_h(x)
        x_w = self.pool_w(x).permute(0, 1, 3, 2)

        # Concatenate and compress channels
        y = torch.cat([x_h, x_w], dim=2)
        y = self.conv1(y)
        y = self.bn1(y)
        y = self.act(y)

        # Split back to original height and width
